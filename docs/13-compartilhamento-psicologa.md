...
- A view compartilhada usa um layout próprio (`layouts/share.blade.php` / `<x-share-layout>`), sem a navegação da área logada (registrar, lembretes, etc.), com um selo fixo "Compartilhado" (ícone de compartilhamento; o tooltip explica que é somente leitura).
