...
>
> **Commit e push só quando pedido explicitamente.** Ao trabalhar em várias mudanças pequenas em sequência (ex.: ajustes de UI, indo e voltando), **não** commitar nem dar push a cada alteração — deixar as mudanças no working tree e só rodar `git add`/`commit`/`push` quando o usuário pedir diretamente (ex.: "comita isso", "pode subir"). Isso evita histórico poluído com commits de "tentativa 1", "tentativa 2" no meio de um ajuste ainda sendo discutido.
...
## 8.3 Troubleshooting — problemas reais encontrados no primeiro deploy

| Sintoma | Causa | Solução |
|---|---|---|
| `MissingAppKeyException` / "headers already sent" | `APP_KEY` vazia no `.env` de produção | `php artisan key:generate --force` + `php artisan config:clear` |
| Log em `storage/logs/laravel.log` prefixado com `local.ERROR` | `.env` de produção com `APP_ENV=local` (esquecido do template de dev) | Editar `.env`: `APP_ENV=production`, `APP_DEBUG=false` |
| Página de login recarrega e mostra e-mail/senha na URL, sem logar | O JS do Livewire não carregou (form caiu no envio nativo do HTML) — ver próxima linha | Ver as duas causas abaixo |
| Erro 405 (`MethodNotAllowedHttpException`) ou tela genérica "Oops! An Error Occurred" | App acessado via URL com subpasta (ex.: `dominio.com/projeto/public`) — o pacote Livewire monta a URL do próprio script e do endpoint `/livewire/update` como caminho absoluto fixo (`/livewire/...`), ignorando `APP_URL`. Isso não é configurável pelo nosso lado. | O app **precisa** responder na raiz de um domínio ou subdomínio, sem nenhuma subpasta na URL (ver seção 8.2 — criar um subdomínio dedicado apontando para `.../public` é o caminho mais simples quando o painel não deixa mudar o document root do domínio principal) |
| `Composer detected issues in your platform: ... PHP version ">= 8.4.1"` mesmo com PHP 8.4 selecionado no painel | A versão de PHP mostrada/selecionada no painel não estava realmente aplicada a esse (sub)domínio (confirmado testando com `curl` num script `<?php echo PHP_VERSION;>`) | Forçar via `public/.htaccess` (`AddHandler application/x-httpd-ea-php84 .php`, já versionado no repo) até o painel corrigir isso, ou até achar a tela certa de MultiPHP por domínio |
| `git pull` recusa com "local changes would be overwritten" em `public/.htaccess` | Alguém editou esse arquivo manualmente direto no servidor (ex.: a correção de PHP acima) e o mesmo arquivo mudou no commit puxado | Se a mudança já foi incorporada ao commit (como o `AddHandler` acima), `git checkout -- public/.htaccess && git pull` |
| Formulário de registro emocional mostra "The mood category id field is required." e não lista nenhum humor | `MoodCatalogSeeder` nunca rodou em produção (fácil de esquecer no meio de uma sessão de troubleshooting de infraestrutura) | `php artisan db:seed --class=MoodCatalogSeeder --force` |
| `manifest.json`, `sw.js` ou ícones dão 404 no console do navegador | Esses caminhos estavam hardcoded como `/manifest.json` etc. no nosso próprio código — corrigido para usar `asset()`/caminhos relativos (ver docs/10, seção 1) | Já corrigido no código; se reaparecer, conferir se o deploy realmente trouxe a versão mais recente (`git log -1`) |
